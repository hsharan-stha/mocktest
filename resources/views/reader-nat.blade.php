            ...
            <div class="question-type-tabs" id="questionTypeTabs">
                @foreach ($questionsByType as $index => $typeGroup)
                    <button class="type-tab {{ $index === 0 ? 'active' : '' }}" 
                        ...
                        <div>
                            {{ $typeGroup['type']->type }}
                            @if(count($typeGroup['sub_sections']) > 0)
                                <span style="font-size: 11px; color: #999; font-weight: normal;">
                                    ({{ count($typeGroup['sub_sections']) }} {{ count($typeGroup['sub_sections']) == 1 ? 'section' : 'sections' }})
                                </span>
                            @endif
                        </div>
        ...
        <div class="exam-main">
            ...
            <div class="exam-header">
                <div class="exam-header-content">
                    <div class="exam-title">NAT Test - Exam</div>
        ...
        <div id="quiz">
            ...
            @foreach ($questionsByType as $typeGroup)
                @foreach ($typeGroup['sub_sections'] as $subSectionGroup)
                    @php
                        // For NAT Test category: Always group questions based on sub-section count or question type count
                        // Display questions per page as per count in sub-section or question type
                        ...
                        if ($subSection && $subSection->count > 0) {
                            // Always group if sub-section has a count
                            $shouldGroupOnSamePage = true;
                        } elseif (!$subSection) {
                            // If no sub-section, check if question type has count
                            // Group questions per page as per question type count
                            $shouldGroupOnSamePage = (($typeGroup['type']->count ?? 0) > 0);
                        }
                    @endphp
                    
                    @if($shouldGroupOnSamePage)
                        ...
                        <div class="question-container quiz-question hidden sub-section-group"
                            ...
                            @if($subSection)
                                <div class="question-section-header">
                                    {{ $subSection->name }}
                                    @if(isset($subSectionGroup['chunk_index']) && $subSectionGroup['chunk_index'] > 0)
                                        <span style="font-size: 14px; font-weight: normal; color: #666;">(Page {{ $subSectionGroup['chunk_index'] + 1 }})</span>
                                    @endif
                                </div>
                            @endif
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateQuestionIndicator(index) {
        // Show current page number and total pages (question containers)
        const totalPages = questions.length;
        document.getElementById("question-indicator").textContent = `Page ${index + 1} of ${totalPages}`;
    }
    ...
    function updateNavigationButtons(index) {
        ...
        // Use questions.length (number of question containers/pages) instead of totalQuestions
        const totalPages = questions.length;
    ...
    function submitExam() {
        ...
        // Calculate score - use allQuestionItems (individual questions) instead of questions (containers)
        score = 0;
    ...
    // Handle option clicks - attach to each question item individually
    allQuestionItems.forEach((questionItem, index) => {
        const optionItems = questionItem.querySelectorAll(".option-item");
        optionItems.forEach(item => {
            item.addEventListener("click", function(e) {
                e.preventDefault();
                e.stopPropagation();
                selectAnswer(index, this.dataset.value);
            });
        });
    });
